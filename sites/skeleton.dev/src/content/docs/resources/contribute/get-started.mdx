...
Skeleton uses [Vite+](https://viteplus.dev/guide/) for local tooling and PNPM workspaces for dependencies.

1. [Install Vite+](https://viteplus.dev/guide/) on your local computer.
...
4. Run `vp install` from the root to install the required dependencies.
5. Point your terminal at one of the `/sites`, `/packages` or `/playgrounds` projects detailed below.
6. Run `vp dev` to start a local dev server for each project.
...
Run the following commands in the monorepo root. `vp run check` runs the package type-check scripts recursively.

| Command        | Description                                    |
| -------------- | ---------------------------------------------- |
| `vp check`     | Checks formatting and lint rules               |
| `vp test`      | Runs all unit tests                            |
| `vp run check` | Runs TypeScript, Svelte, and Astro type checks |
